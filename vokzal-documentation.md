...
### Дизайн интерфейса
#### Окно "Проводник" <a id="design_file_manager"></a>
_Для передачи данных в окно "Проводник" используются [представленные ниже](#file_manager) json-файлы._
...
| Имя |Статус верификации| Описание | Дата изменения | Тип | Автор | Разрешения | Время (Длительность)|
|-----|------------------|----------|----------------|-----|-------|------------|---------------------|
...
#### Окно "Верификация" <a id="design_verification"></a>
_Для передачи данных в окно "Верификация" используются [представленные ниже](#verification) json-файлы._
...
#### Окно "Пользователи" <a id="design_users"></a>
_Для передачи данных в окно "Пользователи" используются [представленные ниже](#users) json-файлы._

Данное окно предназначено для регистрации или редактирования пользователей. 
Также в этом окне можно расставить разрешения для пользователя ([права доступа][2])
либо в ручной настройке, либо установив пакеты разрешений.

_Разрешения и пакеты разрешений являются предположительным решением проблемы прав доступа._


Также, мы не знаем, [нужно ли хранить адрес пользователя или достаточно часового пояса.][4]


![](pictures/polzovateli.png "Окно \"Пользователи\"")


#### Окно "Пользователь" <a id="design_user"></a>
_Для передачи данных в окно "Пользователь" используются [представленные ниже](#user) json-файлы._

Окно открывается при нажатии на имя пользователя справа сверху, в нем можно посмотреть информацию о пользователе
и распечатать её, перейти в чат, посмотреть историю. 
Во фрейме “Новости” отображаются замечания от операторов верификации.

Функции чата, новостей и истории будут добавлены позже.

![](pictures/polzovatel.png "Окно \"Пользователь\"")


#### Окно "Администратор" <a id="design_admin"></a>
_Для передачи данных в окно "Администратор" используются [представленные ниже](#admin) json-файлы._

В данном окне можно регистрировать новые предприятия и создавать системные директории для них
(корневая папка предприятия создается автоматически после добавления предприятия).
Доступно только администратору.

![](pictures/admin.png "Окно \"Администратор\"")


### Структура json-файлов для обмена между клиентом и сервером
#### Json-файлы окна "Проводник" <a id="file_manager"></a>
_[Дизайн интерфейса проводника](#design_file_manager)_

##### Json-файл для фрейма "База данных"
Данный json передается при нажатии на каталог (при первоначальном входе в проводник передается массив каталогов
...
##### Json-файлы для основного окна проводника
    {
    "name": "Email",
...
#### Json-файлы окна "Верификация" <a id="verification"></a>
_[Дизайн интерфейса верификации](#design_verification)_
...
#### Json-файлы окна "Пользователи" <a id="users"></a>
_[Дизайн интерфейса окна "Пользователи"](#design_users)_

##### Json-файл для фрейма "Пользователи"
...
##### Json-файл для основного окна и окна "Пользователь" <a id="user"></a>
...
_[Дизайн интерфейса окна "Администратор"](#design_admin)_
...
[1]: questions.md "1) Как осуществлять логирование действий пользователей?"
...
[4]: questions.md "4) Нужно ли хранить адрес пользователя или достаточно часового пояса?"
